...
set.seed(2)
train <- sample(1:nrow(hdc), 9000)
hdc.test <- hdc[-train,]
hdc.train <- hdc[train,]

tune.linear <- tune(svm, HTATK ~ ., data = hdc.train, kernel="linear", ranges= list(cost=c(0.01, 0.05, 0.1, 0.5, 1, 5, 10)))

tune.radial <- tune(svm, HTATK ~ . -BMICALC -MOD10DMIN -HRSLEEP, data = hdc.train, kernel="radial", ranges= list(cost=c(0.01, 0.05, 0.1, 0.5, 1, 5, 10), gamma = c(0.001, 0.01, 0.1, 1, 10, 100)))

tune.poly <- tune(svm, HTATK ~ . -SALSAMNO -TOMSAUCEMNO -SPORDRMNO, data = hdc.train, kernel="polynomial", ranges= list(cost=c(0.01, 0.05, 0.1, 0.5, 1, 5, 10), degree = c(2, 3, 4, 5)))
...
ggplot(hdc.test, aes(x = HEIGHT, y = BMICALC, color = pred.rad)) +
  ...
  labs(
    ...
    y = "Calculated BMI",
    color = "Report of Heart Attack (1 = Yes)",
    title = "Radial Model Predictions"
  ) +
  scale_color_manual(values = c("steelblue", "coral")) +
  theme_bw()
...
ggplot(hdc.test, aes(x = HEIGHT, y = BMICALC, color = pred.poly)) +
  ...
  labs(
    ...
    y = "Calculated BMI",
    color = "Report of Heart Attack (1 = Yes)",
    title = "Polynomial Model Predictions"
  ) +
  scale_color_manual(values = c("steelblue", "coral")) +
  theme_bw()
```

We can also calculate our test MSE (mean squared error) to get a numerical idea of how accurate our models are.

```{r}
#Test MSE for linear model
mean((pred.lin != hdc.train$HTATK)^2)

#Test MSE for radial model
mean((pred.rad != hdc.train$HTATK)^2)

#Test MSE for polynomial model
mean((pred.poly != hdc.train$HTATK)^2)
